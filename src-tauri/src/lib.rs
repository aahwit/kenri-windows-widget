...
fn place_chat(app: &tauri::AppHandle, focus: bool) -> Result<(), String> {
  ...
  if focus { chat.set_focus().map_err(|e| e.to_string())?; }
  Ok(())
}
...
fn start_chat_follower(app: tauri::AppHandle) {
  thread::spawn(move || {
    ...
    loop {
      ...
      let Some(pet)=app.get_webview_window("main") else { break; };
      let Ok(visible)=pet.is_visible() else { continue; };
      if !visible { continue; }
      ...
      if last != Some(now) {
        ...
        if let Some(chat)=app.get_webview_window("chat") {
          if chat.is_visible().unwrap_or(false) { let _=place_chat(&app,false); }
        }
      }
    }
  });
...
pub fn run(){
 tauri::Builder::default().setup(|app|{
  ...
  tray.on_menu_event(|app,event|match event.id.as_ref(){"show"=>if let Some(w)=app.get_webview_window("main"){let _=w.show();let _=w.set_focus();},"talk"=>{if let Some(w)=app.get_webview_window("main"){let _=w.show();}if let Err(error)=place_chat(app,true){eprintln!("failed to show chat window: {error}");}},"settings"=>if let Some(w)=app.get_webview_window("main"){let _=w.show();let _=w.set_focus();let _=app.emit("open-settings",());},"quit"=>app.exit(0),_=>{}}).build(app)?;Ok(())
...
#[tauri::command] async fn show_chat(app:tauri::AppHandle)->Result<(),String>{place_chat(&app,true)}
