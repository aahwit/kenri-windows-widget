...
fn ensure_chat(app: &tauri::AppHandle) -> tauri::Result<WebviewWindow> {
  ...
  WebviewWindowBuilder::new(app, "chat", WebviewUrl::App("index.html?window=chat".into()))
    ...
    .resizable(false).maximizable(false).decorations(false).transparent(true)
    .always_on_top(true).shadow(false).visible(false).build()
}

fn place_chat(app: &tauri::AppHandle, focus: bool) -> tauri::Result<()> {
  let Some(pet) = app.get_webview_window("main") else { return Ok(()); };
  let p=pet.outer_position()?; let s=pet.outer_size()?;
  ...
  let (x,y,side)=if right<=max_x{(right,p.y.clamp(min_y,max_y),"right")}
    else if left>=min_x{(left,p.y.clamp(min_y,max_y),"left")}
    else if below<=max_y{(p.x.clamp(min_x,max_x),below,"bottom")}
    else{(p.x.clamp(min_x,max_x),above.max(min_y),"top")};
  let chat=ensure_chat(app)?;
  chat.set_position(PhysicalPosition::new(x,y))?;
  let _=chat.emit("bubble-side",side);
  chat.show()?; if focus { chat.set_focus()?; } Ok(())
}

fn toggle_chat_sync(app:&tauri::AppHandle)->tauri::Result<()> {
  let chat=ensure_chat(app)?;
  if chat.is_visible().unwrap_or(false){chat.hide()}else{place_chat(app,true)}
}

fn start_follower(app:tauri::AppHandle){thread::spawn(move||{let mut last=None;loop{thread::sleep(Duration::from_millis(50));let Some(pet)=app.get_webview_window("main")else{break};let Ok(p)=pet.outer_position()else{continue};let now=(p.x,p.y);if last!=Some(now){last=Some(now);if let Some(chat)=app.get_webview_window("chat"){if chat.is_visible().unwrap_or(false){let _=place_chat(&app,false);}}}}});}
...
pub fn run(){tauri::Builder::default().setup(|app|{
 ...
 if let Some(main)=app.get_webview_window("main"){let _=main.set_size(PhysicalSize::new(150,190));let _=main.set_maximizable(false);}
 start_follower(app.handle().clone());
...
}).invoke_handler(tauri::generate_handler![show_chat,hide_chat,toggle_chat]).on_window_event(|window,event|{if let tauri::WindowEvent::CloseRequested{api,..}=event{api.prevent_close();let _=window.hide();}}).run(tauri::generate_context!()).expect("error while running KENRI Desktop Companion");}
